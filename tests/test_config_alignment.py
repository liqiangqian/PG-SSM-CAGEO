...
class ConfigAlignmentTests(unittest.TestCase):
    ...
    def test_locked_defaults_and_candidate_grids(self):
        ...
        self.assertEqual(cfg["stage"]["tau_s"], 0.0)
        self.assertEqual(cfg["stage"]["Delta_max"], 0.80)
        self.assertEqual(cfg["model"], {"hidden_size": 64, "dropout": 0.10, "latent_fusion": "addition"})
        self.assertEqual(cfg["optimizer"]["learning_rate"], 1e-3)
